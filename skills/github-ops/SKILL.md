...
After completing the GitHub operations, suggest the natural next steps to the user:

```
[Summary of GitHub operation completed].

Options:
A) webapp-testing — Propose running automated testing scripts on the active branch/PR.
B) fact-checker — Propose verifying factual claims in newly edited or updated documents.
C) No thanks — The repository state is fine as-is.
```
