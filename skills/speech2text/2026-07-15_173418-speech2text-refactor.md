...
4. **ffprobe for accurate ETA** — Resolved: use Python stdlib `wave.open(wav, 'rb')` to read the header (`getnframes() / getframerate()`) for exact seconds. Zero external process, zero third-party dep. Wraps in try/except: if the file is non-PCM WAV (rare) or an mp4/mp3 in single-file mode, fall back to `os.path.getsize / 32000` heuristic (assumes mono-16k-s16le, off by up to 4x on stereo-44k). Real transcribe mode always uses `info.duration` from Whisper, so ETA precision matters only for dry-run.
