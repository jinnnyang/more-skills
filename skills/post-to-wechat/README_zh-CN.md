...
3. **极致封面设计与点击率（打开率）优化体系**
   - **“封面即生死”法则**：在技能中深度强调封面设计是公众号文章“打开率”的第一决胜点。粗制滥造、缺乏设计感或充满廉价AI感的封面会大幅降低读者点开的兴趣，使文章流量直接夭折。
   - **6维度设计矩阵**：深度整合并强化了 `baoyu-cover-image` 的 5+1 维设计理论（视觉类型、配色方案、渲染风格、文字密度、情绪色彩、字体设计）。要求智能体坚决杜绝廉价网图，必须采用极简且具视觉冲击力的**隐喻化设计（Visual Metaphor）**，配以 40%~60% 的留白和极具高级感的色彩搭配，打造具备极强吸睛力的高品质封面。
   - **比例与安全区控制**：强制首图使用微信官方的 `2.35:1` 黄金比例，次图使用 `1:1` 比例，从技术与美学双重维度确保在各种手机端信息流中完美呈现，杜绝文字截断或比例拉伸。
4. **自动压缩与体积优化管道**
...
11. **顶层战略与内容排期规划 (WeChat Native)**
    - 在技能中融入了针对微信平台的运营战略，强制推行 60/30/10 内容黄金配比法则（价值内容/互动运营/推广转化），结合微信动态标签维护精准的用户画像，并设定了具体的业务提升目标（如打开率 >30%，完读率 >50%）。
12. **自动化闭环日志与数据反哺机制**
    - 强制要求在群发或保存草稿成功后，将发布结果（PostId、链接等）结构化写入指定的 `publish-logs/` 日志库。该设计确保了数据分析智能体（Data Assistant）能够动态抓取长尾数据并自动调优下一周期的内容配比策略。
