...
function parseWechatExtend(content: string): WechatExtendConfig {
  ...
  for (const raw of lines) {
    ...
    switch (key) {
      ...
      case "preferred_image_backend": config.preferred_image_backend = val; break;
      case "preferred_backend": config.preferred_image_backend = val; break;
      case "auto_compress": config.auto_compress = val === "true" || val === "1"; break;
      case "max_cover_size_mb": config.max_cover_size_mb = parseFloat(val); break;
    }
  }
...
export function loadWechatExtendConfig(): WechatExtendConfig {
  const p = path.join(skillDir, "EXTEND.md");
  try {
...
export function resolveAccount(config: WechatExtendConfig, alias?: string): ResolvedAccount {
  ...
  return {
    ...
    preferred_image_backend: config.preferred_image_backend,
    auto_compress: config.auto_compress,
    max_cover_size_mb: config.max_cover_size_mb,
  };
}
...
function resolveCredentialSource(
  ...
  for (const source of sources) {
    if (source.appId && source.appSecret) {
      if (source.name.startsWith("EXTEND.md account")) {
        console.error(
          `[WARNING] Credentials resolved from: ${source.name}. Storing credentials in EXTEND.md is not recommended for security reasons as it might easily be committed. Please use environment variables or a local .env file instead.`
        );
      }
      return {
        appId: source.appId,
  ...
  throw new Error(
    ...
    "Set via EXTEND.md account config, environment variables, or skills/post-to-wechat/.env file." +
    partialHint
  );
}
...
export function loadCredentials(account?: ResolvedAccount): LoadedCredentials {
  ...
  if (prefix) {
    ...
    sources.push(
      ...
      buildCredentialSource(`skills/post-to-wechat/.env (${prefixedKeyLabel})`, skillEnv, `${prefix}APP_ID`, `${prefix}APP_SECRET`),
  ...
  sources.push(
    ...
    buildCredentialSource("skills/post-to-wechat/.env", skillEnv, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
