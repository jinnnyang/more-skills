import { BingClient, SearchResult } from './engines/cn.bing.com.js';
...
async function main() {
  ...
  let engineOpt: string | null = null;
  let format = 'raw';
  ...
  const enginesToRun: { name: string; ClientClass: any }[] = [];
  if (engineOpt) {
    ...
    const uniqueClasses = new Map<any, string>();
  ...
  try {
    if (format === 'raw') {
      ...
      for (const { name, ClientClass } of enginesToRun) {
        ...
        for (const query of queries) {
          try {
            const results: SearchResult[] = await client[methodName](query);
