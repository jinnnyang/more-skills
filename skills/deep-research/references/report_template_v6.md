...
数据可视化与多媒体引用规则：
- **外部图表直插**：在正文叙述中，若 KANBAN 多媒体注册表中有高度匹配当前论点的外部图表 URL，必须直接在段落间使用 Markdown 格式（`!["详尽alt"](URL "Title")`）将其嵌入作为视觉佐证，不要只堆在附录。
- **内部绘图**：优先检测并调用绘图工具生成 PNG 图片并存入 assets/ 目录，使用标准 markdown 格式嵌入。
- **降级绘制**：若无专门绘图工具，则回落为 Mermaid.js 代码块，例如：
  ```mermaid
  flowchart TD
      A["A 节点"] --> B["B 节点"]
  ```
...
## 智能体洞察与研判 / AI Insights & Synthesis

- **洞见 1:** [基于上方事实证据与图表，提炼出大模型独立推演的供应链瓶颈、政策影响或未来趋势]
- **洞见 2:** [强调区别于纯事实罗列的升维观点，需包含明确的因果推论]
