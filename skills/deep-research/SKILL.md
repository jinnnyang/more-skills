...
Lead Agent (主路由入口 — 最小化上下文占用)
  │
  ├── [P0] 环境核对与工具/技能主动发现 ──→ 载入 references/phases/p0_setup.md
  │    └── 🔍 扫描并向用户列出就绪的 MCP 工具与工作区专属技能，初始化 KANBAN.md 看板
  │
...
2. **环境主动感知**：在 P0 阶段，必须探测当前可用的 MCP 服务与已就绪的兄弟技能，主动展示并告知用户已关联的辅助分析工具。
