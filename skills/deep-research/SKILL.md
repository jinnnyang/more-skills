...
Lead Agent (coordinator — minimizes raw search context)
  |
  P0: Environment check (check_env.py) & Interactive Intake
  |   (git init, create folders: assets/, findings/, tasks/, plan/, kanban/)
  |
  P1: Research Task Board Setup -> [git commit: stage: plan-initialized]
  |
  Dispatch ──→ Subagent A ──→ writes findings/task-a.md ──┐
           ──→ Subagent B ──→ writes findings/task-b.md ──┤ (parallel)
           ──→ Subagent C ──→ writes findings/task-c.md ──┘
  |                                                        |
  |     findings/  <───────────────────────────────────────┘
  |
  P2: Dispatch & Investigate notes complete -> [git commit: stage: research-notes-completed]
  |
  P3: Citation Registry & Media Localization & Tool Discovery Drawing Fallback
  |
  P4: Evidence Outline & Diagram Planning -> [git commit: stage: registry-outline-built]
  |
  P5: Draft from Notes (with localized relative images/Mermaid) -> [git commit: stage: draft-report-written]
  |
  P6: Counter-review (claims, confidence, alternatives)
  |
  P7: Verify, Polish & Finalize -> [git commit: stage: report-finalized]
```
...
## P0: Environment Setup & Interactive Intake

### 1. 自动化环境检查 (Environment Check)
启动前，必须自动执行脚本 `python scripts/check_env.py` 并检查其报告。
- 若报告网络、写权限等关键环境失败（`FAILED`），必须先解决依赖（如利用 `uv` 安装必要库）或报告错误并中止。
- 只有在关键检查项（网络连接、磁盘写权限、Git 可用性）满足后才可继续。

### 2. 交互式需求确认 (Interactive Intake)
Lead Agent 结合用户调研请求，自动生成 **3 个左右候选选项**（包括研究主题、研究边界/范围、短横线项目根目录命名），并向用户提出单选/多选题，严禁填空提问。确认后建立项目工程（例如项目名为 `rare-earth-export-control-study`）。

### 3. 项目工程化结构初始化
在指定 `projects/` 目录下创建以项目名命名的目录，执行以下操作：
1. 在项目根目录下运行 `git init` 进行版本控制。
2. 创建以下五个核心子文件夹：
   - `assets/`（存放多媒体佐证及绘图图片）
   - `findings/`（存放子任务 notes）
   - `tasks/`（存放任务定义）
   - `plan/`（存放调研大纲与计划）
   - `kanban/`（存放看板状态）
3. 写入初始 `README.md` 和 `README_zh-CN.md`。
4. 初始化 `kanban/project_state.json` 以保存最初的 `project_info` 和 `kanban`。

### 4. 策略变量定义
设置策略变量：
- `AS_OF`: 今天的日期 (YYYY-MM-DD)
- `MODE`: Standard 模式或 Lightweight 模式
- `SOURCE_TYPE_POLICY`: 强制进行 official/academic/secondary 等分类
- `COUNTER_REVIEW_PLAN`: 反方观点审查策略

Report: `[P0 complete] Environment Checked: ok. Project Directory Initialized: yes. AS_OF: {YYYY-MM-DD}.`
...
- **Output**: Path to research notes file (under `findings/` directory)
...
### 看板更新与 Git 自动提交
将规划好的任务列表序列化写入 `kanban/project_state.json` 中的 `kanban.tasks` 字段，并在根目录下执行 Git 自动提交：
```powershell
git add .
git commit -m "stage: plan-initialized"
```

Report: `[P1 complete] {N} tasks in {M} groups. Dispatching Group A. Git Stage: plan-initialized.`
...
### 抓取完成与 Git 自动提交
当所有子代理的任务笔记汇总写回至 `findings/` 目录下后，Lead Agent 更新 `kanban/project_state.json` 中的任务状态，并在项目根目录下执行 Git 自动提交：
```powershell
git add .
git commit -m "stage: research-notes-completed"
```

Status per task: `[P2 task-{id} complete] {N} sources, {M} findings.`
Status all: `[P2 complete] {N} tasks done, {M} total sources. Git Stage: research-notes-completed.`
...
## P3: Citation Registry, Source Governance & Media Localization

Lead agent reads all task notes, builds a unified registry, localizes multimedia evidence, and plans for data visualizations.

### P3 Process

1. **Build Citation Registry**:
   - Read every task file's `## Sources` section.
   - Merge all sources, deduplicate by URL, and assign sequential global [n] numbers.
   - Tag with `source_type`, `as_of`, `authority` score, and apply quality gates (Standard: ≥12 approved, ≥5 domains, ≥30% official; Lightweight: ≥6 approved, ≥3 domains, ≥20% official).
   - Drop low-quality sources.

2. **Multimedia Localization**:
   - Parse all `findings/task-*.md` files to extract image Markdown links.
   - Download the high-value images to the project's `assets/` folder, using the naming format: `assets/[Task_ID]_[Timestamp]_[Hash].[ext]`.
   - Update the `media_registry` in `kanban/project_state.json`. If a download fails, fall back to the original URL and log a warning.

3. **Diagram Tool Discovery**:
   - Check if there are other specialized drawing/plotting skills registered in the environment (e.g. standard plotting libraries, canvas skills).
   - If available, prioritize using them to generate diagrams and save them as PNGs in `assets/`.
   - If not available, plan to use inline Mermaid diagrams following the templates in `references/visualization_mermaid_guide.md`.

4. **Update Project State**:
   - Write the finalized approved/dropped citations and media registries into `kanban/project_state.json`.
...
6. **Plan Visualizations**: Specify which sections will have diagrams (e.g., SWOT, process flow) and whether specialized drawing tools or Mermaid will be used.
...
Diagram plan: {Mermaid type or drawing tool asset target}
Gaps: {limited official evidence}
```

### 大纲完成与 Git 自动提交
在项目看板中登记大纲及绘图计划，并在根目录下执行 Git 自动提交：
```powershell
git add .
git commit -m "stage: registry-outline-built"
```
...
6. **Media and Diagram Verification**: Ensure all images referenced in the markdown report exist in `assets/` and render correctly. Verify all Mermaid syntax is error-free.
7. **README Update**: Summarize key findings in `README.md` and `README_zh-CN.md`.

### 项目归档与 Git 最终提交
在 `kanban/project_state.json` 中标记项目状态为 `COMPLETED`，并在根目录下执行最终的 Git 提交：
```powershell
git add .
git commit -m "stage: report-finalized"
```

Report: `[P7 complete] {N} spot-checks, {M} violations fixed. Git Stage: report-finalized. Project status: COMPLETED.`
...
| [project_management_guide.md](references/project_management_guide.md) | **P0/P1**: Project layout, Git tracking, interactive intake, JSON state |
...
| [multimedia_collection_guide.md](references/multimedia_collection_guide.md) | **P3**: Media asset scanning, filtering, download, relative embedding alt text |
| [visualization_mermaid_guide.md](references/visualization_mermaid_guide.md) | **P4/P5**: Plotting skill discovery, Mermaid template guidelines, syntax error-proofing |
| [report_template_v6.md](references/report_template_v6.md) | P5: Draft with confidence markers, counter-review, and media appendix |
