...
4. **Persistent Kanban State Tracking**: Uses `kanban/KANBAN.md` to record and update stages, subagent tasks (with recursive parameters), global validated citations, and media registries, supporting research resume and pause cycles.
...
[project-name]/               # Project Root Directory (kebab-case)
  ...
  ├── memories/              # Historical context and factual background memory (task-*.md)
  ├── findings/              # Raw objective findings and notes written by subagents (task-*.md)
  ├── clues/                 # Unverified leads or potential exploration paths (task-*.md)
  ├── hypotheses/            # Analytical deductions and logical assumptions generated by agents (task-*.md)
