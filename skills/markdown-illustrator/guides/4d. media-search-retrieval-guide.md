...
## 📰 三、 事实/新闻类网络检索与免下载配图规范 (News & Factual Web Search)

当文章涉及突发新闻、历史事实或时政信息（如"SpaceX 上市"、"Trump 访华"、"爪哇岛地震"）时，如果运行环境支持浏览器（`UseBrowser = true`），应启动浏览器检索相关的新闻大图。为了保持新闻时效性并遵守版权分发限制，**此类配图采用"远程热链 (Hotlink)"方案，无需下载物理文件**。

### 1. 运行能力前置判定
在执行网络检索前，Agent 必须读取 `[原文章名].plan.md` 中的 `* **环境浏览器能力 (UseBrowser)**` 状态。
*   若为 `false`：直接**跳过**网络检索，仅执行本地资产池匹配，或者提示用户进行手动资产注入。
*   若为 `true`：开启浏览器执行以下网络搜索流。

### 2. 检索与校验执行流

1. **上下文实体提取与查询构建 (Query Formulation)**：
   - 分析插图锚点前后 2-3 个段落，提取核心的"时间-地点-人物-事件"实体。
   - **语言判定规则**：依据主实体语言特性选择最适合的查询语言（如 `SpaceX`、`NASA` 优先使用英文，`全国两会`、`神舟发射` 优先使用中文）。
   - **搜索引擎选择**：
     - 中文查询使用：`https://cn.bing.com/images/search?q={query}&qft=+filterui:photo-photo`
     - 英文查询使用：`https://www.bing.com/images/search?q={query}&qft=+filterui:photo-photo`
     - *提示*：`qft=+filterui:photo-photo` 参数能过滤掉剪贴画、矢量插画和概念图，只显示纪实照片。

2. **浏览器页面交互与图片提取 (Browser Interaction Loop)**：
   - 使用浏览器工具访问构建好的搜索 URL。
   - **大图/原图提取交互流**：由于搜索主页仅提供低清缩略图，Agent 必须执行以下循环交互：
     1. **遍历缩略图**：定位搜索结果列表中的缩略图元素。
     2. **触发弹窗 (Click Thumbnail)**：模拟点击缩略图以触发页面上的大图弹窗 (Lightbox)。
     3. **提取大图 URL**：在大图展示弹窗加载完毕后，提取弹窗中展示的**原始高清大图 URL (Original Image URL)** 和图片自带的 alt 描述或新闻上下文。
     4. **关闭弹窗 (Close Lightbox)**：点击关闭弹窗按钮或执行返回，恢复结果列表状态。
     5. **循环处理**：重复以上操作，提取前 3-5 张符合条件的候选大图。
     - *注*：由于页面 DOM 结构可能会随搜索引擎更新而调整，上述步骤属于策略性交互引导，Agent 需根据运行时的网页结构，灵活使用选择器定位大图弹窗和关闭按钮。

3. **官媒定向过滤 (Authority Filtering)**：
   - 过滤提取出来的候选图片，优先选择权威机构或官方主流新闻媒体的图片源（如新华社、路透社、NASA 官网、TechCrunch 等）。
   - 严禁引用微博、贴吧等个人社交媒体或博客网站的二次转载图。

4. **多模态视觉核验 (Multimodal Vision Inspection)**：
   - **核心步骤**：Agent 必须使用多模态大模型的"图片阅读/视觉感知"功能，传入图片 URL 直接阅读其具体画面内容。
   - **审查重点是"防伪与防穿帮"**：
     - **实体准确性**：画面是否确实包含提取的实体？（如：图片中是否确实是 SpaceX 发射场？）
