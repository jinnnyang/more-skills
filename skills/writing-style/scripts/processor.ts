...
export async function processMarkdown(
  ...
  // BUGFIX: md2ast parser cannot correctly handle images nested inside links ([![...](...)](...))
  // It breaks them into multiple text nodes across parent/children (image opening inside link, closing outside)
  // The fix: completely rebuild children nodes for any container that contains image markdown syntax in text
  const imageRegex = /!\[([^\]]*)\]\(([^)]+?)(?:\s+"([^"]+)")?\)/g;
  
  // Get all text recursively from node and its children
  function getAllTextRecursive(node: AnyNode): string {
  ...
  function fixMisparsedImagesInNode(node: AnyNode, parent: AnyNode): void {
    // Recursively fix all children first
    if ('children' in node && Array.isArray(node.children) && node.children.length > 0) {
      for (const child of [...node.children]) { // iterate over copy
        fixMisparsedImagesInNode(child, node);
      }
    }
    
    // For any container node, get the complete text including all nested children
    const fullText = getAllTextRecursive(node);
    
    // Check if there are any image matches in the complete text
    imageRegex.lastIndex = 0;
    let hasMatch = false;
    let matchCount = 0;
    while (imageRegex.exec(fullText)) {
      hasMatch = true;
      matchCount++;
    }
    imageRegex.lastIndex = 0;
    ...
    // There are images here — we need to completely rebuild the entire node's content
    // from the full text because the original parsing is broken. All nested structure is lost
    // but since we only have text + images inside links, this is acceptable for markdown normalization.
    ...
    while ((match = imageRegex.exec(fullText)) !== null) {
      if (match.index > lastIndex) {
        ...
        newChildren.push({
          type: 'text',
          value: fullText.slice(lastIndex, match.index)
        } as AnyNode);
      }
      
      // Create proper image node from the match
      ...
      // Try to get line number from original node
      if ('line_num' in node) {
        imageNode.line_num = (node as any).line_num;
      }
      newChildren.push(imageNode);
      
      // Add to processing queue if needed
      if (!imageNode.alt || imageNode.alt.trim().length < 5 || !imageNode.title) {
        let contextText = '';
        if (typeof imageNode.line_num === 'number') {
          const start = Math.max(0, imageNode.line_num - 5);
          const end = Math.min(lines.length - 1, imageNode.line_num + 5);
          const contextLines = lines.slice(start, end + 1).filter((_, i) => (start + i) !== imageNode.line_num);
          contextText = contextLines.join('\n').trim();
        }
        imagesToAnalyze.push({ img: imageNode, contextText });
      }
    ...
    if (lastIndex < fullText.length) {
      newChildren.push({
        type: 'text',
        value: fullText.slice(lastIndex)
      } as AnyNode);
    }
  ...
  if (ast.children) {
    for (const child of ast.children) {
      fixMisparsedImagesInNode(child, ast);
    }
  }
